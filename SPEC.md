...
### 3.6 `key_constraints` and `invariants` — scope note for v0.1 → v0.2

In Architecture Decision Record literature (Nygard 2011, MADR) and the source `decision-protocol-template.md` motivating EDP, the field carrying a decision's binding constraints is called **`invariants`** and is required to be programmatically assertable — a runtime predicate that a verifier agent checks against every planned action BEFORE execution. The source labels invariants "the most important field" and states bluntly: *without programmatic invariants and a verifier, decision drift is guaranteed.*

EDP v0.1 ships **`key_constraints: list[str]`** (natural-language, ≤140 chars each) as the agent-visible binding statements, plus the **soft `edp_check(planned_action)` advisory** (lexical FTS5 match) for self-consultation. This is a deliberate scope-narrowing for v0.1, not an abandonment of the verifier-gate model. The motivations are: (a) visibility-first — making decisions present in the agent's working context every turn is itself a measurable intervention, and EDP v0.1 is the experiment that quantifies its effect; (b) extension-shaped — implementations are free to layer a pre-action verifier on top of the v0.1 store and tools without forking the spec, since the data model already carries every field a verifier would consume.

The visibility-via-snippet-block primitive in v0.1 is hypothesised to outperform the visibility-via-`CLAUDE.md` primitive that the README documents as failing in practice (e.g. claude-code#37550, cline#4208) because of four specific differences: per-turn re-injection vs static prefix that the compactor can evict; supersede chain with explicit pointers vs in-place edits that lose history; structured `key_constraints` enumerable by the agent vs prose mixed with other guidance; and an agent-callable `edp_check` vs no programmatic re-read path. Whether this hypothesis holds at scale is an open empirical question; the v0.1 release is a calibration step toward answering it.

**v0.2 verifier extension point — advisory, not enforcement.** A `verifier_hook(planned_action: str, active_decisions: list[Decision]) -> CompatibilityReport` extension lands in v0.2. `CompatibilityReport` is `{verdict: "compatible" | "violated" | "uncertain", reasoning: str, violated_decision_ids: list[str] | None}`. When wired into a harness's pre-tool-use hook (PreToolUse in Claude Code; equivalent in other harnesses) and backed by a cheap model (Haiku-class), this gives implementations a cheap second-opinion check before action execution. The v0.2 reference implementation also introduces the optional `invariants: list[str]` field alongside `key_constraints` — machine-targeted predicates the verifier consumes; expressed in natural language with a docstring contract that each entry SHOULD be assertable. The hook is opt-in (`edp claude-code install --enable-verifier`) and stays opt-in by design.

**Honest framing — the verifier is advisory, NOT a security boundary.** The agent itself writes its own invariants (via `edp_record`) and its own planned actions (via the tool calls the verifier inspects). A coordinated agent can always escape the check by rewriting either side, and the verifier prompt's untrusted-content delimiters cannot fully prevent prompt-injection attacks against the cheap-model checker. The verifier is a useful catch for *unintentional* drift (the much more common failure mode); it is not a defence against *intentional* circumvention. See `.planning/research/verifier-audit-2026-05-25.md` for the threat-model analysis. Implementations MUST surface this framing to operators — the v0.2 reference adapter prints a data-egress + advisory-only notice in the install output when `--enable-verifier` is chosen.
