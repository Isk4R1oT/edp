...
**v0.2 verifier extension point.** A `verifier_hook(planned_action: str, active_decisions: list[Decision]) -> CompatibilityReport` extension is on the v0.2 roadmap. `CompatibilityReport` is `{verdict: "compatible" | "violated" | "uncertain", reasoning: str, violated_decision_ids: list[str] | None}`. When wired into a harness's pre-tool-use hook (PreToolUse in Claude Code; equivalent in other harnesses) and backed by a cheap model (Haiku-class), this lets implementations enforce decisions at the action gate per the source-doc design. The v0.2 reference implementation will introduce the optional `invariants: list[str]` field alongside `key_constraints` (machine-checkable predicates intended for verifier consumption; expressed in natural language but with a docstring contract that each entry must be assertable). The hook is opt-in for v0.2 (`edp claude-code install --enable-verifier`) and on-by-default for v0.3 once the visibility-vs-gating effect has been measured.
